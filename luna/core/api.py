...
class Api(MethodView):
    ...
    @classmethod
    def register_app(cls, app, prefix='/api'):
        rule = cls._build_api_rule(prefix=prefix)
        ...
        for name, member in members:
            ...
            rule = cls._build_api_rule(rule=member.rule, prefix=prefix)
            options = member.options
            endpoint = '::'.join([cls.__name__, func.__name__])
            app.add_url_rule(rule, endpoint, func, **options)

    @classmethod
    def _build_api_rule(cls, rule=None, prefix=None):
